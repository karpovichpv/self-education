...
world.scene.setup();
world.scene.config.backgroundColor = new THREE.Color('#f0f0f0');
world.scene.config.directionalLight.intensity = 0.5;
world.scene.config.ambientLight.intensity =0.5; 
...
async function loadIfc() {
  ...
  const currentProperties = await model.getProperties;
...
const panel = BUI.Component.create<BUI.PanelSection>(() => {
  return BUI.html`
  <bim-panel active label="Настройки просмотра" class="options-menu">
      <bim-panel-section style="padding-top: 12px;">
      
        <bim-button label="Загрузить IFC"
          @click="${() => {
            loadIfc();
          }}">
        </bim-button>  
        <bim-color-input 
          label="Цвет фона" color="#f0f0f0" 
        ...
        <bim-number-input 
          slider step="0.1" label="Интенсивность прямого света" value="0.5" min="0.1" max="2"
        ...
        <bim-number-input 
          slider step="0.1" label="Интенсивность освещенности поверхностей" value="0.5" min="0.02" max="2"
          @change="${({ target }: { target: BUI.NumberInput }) => {
            world.scene.config.ambientLight.intensity = target.value;
          }}">

        </bim-number-input>
                <bim-checkbox label="Видимость сетки" checked 
        ...
        <bim-number-input 
          slider step="0.1" label="Размер главной сетки" value="1" min="0" max="10"
          @change="${({ target }: { target: BUI.NumberInput }) => {
        ...
        <bim-number-input 
          slider step="0.1" label="Размер второстепенной сетки" value="10" min="0" max="20"
          @change="${({ target }: { target: BUI.NumberInput }) => {
